...
from sqlalchemy import String, Text, BigInteger, ForeignKey, JSON, Numeric, Integer, Boolean, DateTime
...
class Agent(BaseModel):
    ...
    name:Mapped[str] = mapped_column(String(200),comment="Agent名称")
    description:Mapped[str | None] = mapped_column(Text,nullable=True,comment="Agent描述")
    ...
    model_id:Mapped[int | None] = mapped_column(BigInteger,ForeignKey("models.id",ondelete="SET NULL"),
                                                nullable=True,comment="模型ID")

    # 核心配置
    config:Mapped[dict | None] = mapped_column(JSON,nullable=True,comment="Agent 核心配置")
    # 运行统计
